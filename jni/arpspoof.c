...
int main(int argc, char *argv[])
{
	...
	printf( "dSploit ArpSpoofer - emeykey fork v1.1a (Android 6-11).\n\nUsage: arpspoof [-i interface] [-t target] host\n\n" );
	...
	// emeykey START
	// Get MAC string from ip a
	FILE *fp;
	unsigned char path[150];

	fp = popen("ip a | grep -v swlan | grep wlan0 -A 1 | grep link | sed -e 's/.*link\\/ether //g' | sed -e 's/ brd.*//g'", "r");

	while (fgets(path, sizeof(path), fp) != NULL) {
	}

	// Convert MAC from string to hex string of 6 bytes
	unsigned int iMac[6];
	unsigned char our_mac[6];
	int im;

	sscanf(path, "%x:%x:%x:%x:%x:%x", &iMac[0], &iMac[1], &iMac[2], &iMac[3], &iMac[4], &iMac[5]);
	for(im=0;im<6;im++)
    	our_mac[im] = (unsigned char)iMac[im];
	...
	if( our_mac == NULL )
	{
		printf( "[ERROR] Unable to retrieve local hardware address.\n");
		return 1;
	}
